...
mpu6050_return_status mpu6050_find_or_check_device(mpu6050_user_configs* config){
	...
	if(i2c_manager_check_device(config->i2c_handle, config->i2c_addr, addr_who_am_i, 0x70) == _i2c_manager_ok){ // bizim mpu6050 0x70 döndürüyor
		return _mpu6050_ok;
	}
...
volatile uint32_t mpu6050_irq_count = 0;
